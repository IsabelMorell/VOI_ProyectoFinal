...
if __name__ == "__main__":
    ...
    output_path = os.path.join(output_folder_path, "output_video_procesos5.avi")
    ...
    if correct_password:
        ...
        while not win:
            ...
            # Comienzo la sustraccion de fondo en tiempo real
            mask = mog2.apply(ball_mask)  # Esto es lo que se ha movido (osea la pelota)

            # Calcular el gradiente entre la mask y la mask anterior para saber si la pelota esta bajando o subiendo
            coords = np.column_stack(np.where(mask > 0))  # Pixeles azules que se han movido
            if coords.size > 0 and coords.size < 255:
                x = np.mean(coords[:, 1])
                y = np.mean(coords[:, 0])
                ...
                if saque and ((turn_player1 and x > left_net) or ((not turn_player1) and x < right_net)):
                    saque = False
                    print("num_bounces antes del punto 9", num_bounces)
                    if num_bounces == 0:
                        if turn_player1:
                            score2 += 1
                            print("score2:", 9)
